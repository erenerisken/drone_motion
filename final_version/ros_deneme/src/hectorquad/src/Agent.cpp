...
namespace RVO {
	Agent::Agent(const std::string &id, float radius) : maxNeighbors_(20), maxSpeed_(MAX_SPEED), neighborDist_(5.0f), radius_(radius), timeHorizon_(0.1f), timeHorizonObst_(0.23f), id_(id) { }
	//leak check
	void Agent::prepareStep(const gazebo_msgs::ModelStates &neighbors, const geometry_msgs::Twist &pref)
	{
		...
		velocity_.x_ = neighbors.twist[0].linear.x;
		velocity_.y_ = neighbors.twist[0].linear.y;
		prefVelocity_.x_ = pref.linear.x;
		prefVelocity_.y_ = pref.linear.y;
		//ROS_WARN_STREAM("Prepare Step ended.");
		obstacleNeighbors_.clear();//TODO
		obstacles.clear();
		/*for(size_t i = agentNeighbors_.size()-1; i>=0; --i)
			{
		...
		for (size_t i = 1; i < neighborCount; i++)
			{
				Agent *tmp = new Agent(neighbors.name[i], COLLISION_RADIUS);
				...
				tmp->velocity_.y_ = neighbors.twist[i].linear.y;
				agentNeighbors_.push_back(std::make_pair(absSq(position_ - tmp->position_), tmp));
			}
		//computeNewVelocity();
	}
	void Agent::insertObstacleNeighbor(Obstacle *obstacle)
	{
		Obstacle *nextObstacle = obstacle->nextObstacle_;
		const float rangeSq = 10000.0f;
		//ROS_INFO_STREAM("Measuring Distance");
		if(nextObstacle == NULL)
			ROS_ERROR_STREAM("Next is null..");
		float distSq = distSqPointLineSegment(obstacle->point_, nextObstacle->point_, position_);
		//ROS_INFO_STREAM("Measured Distance");
		if (distSq < rangeSq) {
			obstacleNeighbors_.push_back(std::pair<float, Obstacle*>(distSq, obstacle));
	...
	size_t Agent::addObstacle(const std::vector<Vector2> &vertices)
	{
		//ROS_WARN_STREAM("Adding Obstacle");
		if (vertices.size() < 2) {
			return RVO_ERROR;
		}
		...
		for (size_t i = 0; i < vertices.size(); ++i) {
			...
			//ROS_WARN_STREAM("Added Obstacle");
		}
		for (size_t i = obstacleNo; i < obstacles.size(); i++)
			{
				insertObstacleNeighbor(obstacles[i]);
			}
	...
	void Agent::computeNewVelocity()
	{
		...
		for (size_t i = 0; i < agentNeighbors_.size(); ++i) {
			...
			if (distSq > combinedRadiusSq) {
				...
				const float invTimeStep = 1.0f / TIME_STEP;
